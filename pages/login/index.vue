    ...
    <view class="content">
        ...
        <view class="login-bg">
            <view class="login-card">
                ...
                <view v-if="type=='login'">
                    ...
                    <label class="uni-list-cell uni-list-cell-pd">
                        <label class="label-2-text">
                            <image src="../../static/img/icon_user.png" class="icons" style="width: 20px;height: 20px;">
                            </image>
                        </label>
                        <view>
                    ...
                    <label class="uni-list-cell uni-list-cell-pd">
                        ...
                        <uni-icons style="display: none;" class='tip-icons' type="eye-filled"
                            :class="[showPassword ? '' : 'active']" @tap="changePassword"></uni-icons>
                ...
                <view v-if="type=='reg'">
                    ...
                    <label class="uni-list-cell uni-list-cell-pd">
                        ...
                        <view>
                            <input class="uni-input" maxlength="11" placeholder="请输入用户名" v-model="form.username"
                                @blur="check_name($event.detail.value)" />
                        </view>

                        <uni-icons :type="usernamemsg=='ok'?'checkbox-filled':'clear'"
                            :class="{'tip-icons':true,'ok':usernamemsg=='ok','error':usernamemsg=='error'}"></uni-icons>
                    </label>
                    ...
                    <label class="uni-list-cell uni-list-cell-pd">
                        ...
                        <view>
                            <input class="uni-input" placeholder="请输入登录密码" :password="showPassword"
                                v-model="form.password" @input="checkpwd($event.detail.value)"
                                @blur="check_other($event.detail.value,'password')" />
                        </view>
                        <uni-icons :type="passwordmsg=='ok'?'checkbox-filled':'clear'"
                            :class="{'tip-icons':true,'ok':passwordmsg=='ok','error':passwordmsg=='error'}"></uni-icons>
                    ...
                    <label class="uni-list-cell uni-list-cell-pd">
                        ...
                        <view>
                            <input class="uni-input" placeholder="请再次输入密码" :password="showPassword"
                                v-model="form.password2" @blur="check_other($event.detail.value,'password1')" />
                        </view>
                        <uni-icons :type="passwordmsg2=='ok'?'checkbox-filled':'clear'"
                            :class="{'tip-icons':true,'ok':passwordmsg2=='ok','error':passwordmsg2=='error'}"></uni-icons>


                    </label>
                    <label class="uni-list-cell uni-list-cell-pd" v-if="system.inviteopen==1">
                        <label class="label-2-text" style="width:100upx;">
                            <image src="../../static/img/icon-tip.png" class="icons" style="width: 20px;height: 20px;">
                            </image>
                        </label>
                        <view>
                    ...
                    <label class="uni-list-cell uni-list-cell-pd">
                        ...
                        <view>
                            <input class="uni-input captch" maxlength="4" placeholder="输入右侧验证码" v-model="form.captch"
                                @blur="check_other($event.detail.value,'randcode')" />
                        </view>
                        <image :src="codeurl" class="captchimg" @tap="changecode()"></image>
                        <uni-icons :type="captchmsg=='ok'?'checkbox-filled':'clear'"
                            :class="{'tip-icons':true,'ok':captchmsg=='ok','error':captchmsg=='error'}"></uni-icons>
            ...
            <view class="login-btn">
                ...
                <button v-if="type=='login'" class="landing_true" :disabled="checkIn ? false : true" type="primary"
                    @tap="click_login">立即登录</button>
                <button v-else class="landing_true" :disabled="checkIn ? false : true" type="primary"
                    @tap="click_reg">确认并注册</button>
            </view>
        ...
        <view style="text-align: center;margin-top: 30px;">
            <checkbox v-bind:checked="isread" @tap="isread=!isread" id="readrule" value="1">我已阅读并同意</checkbox>《<view
                class="rule" @tap="go_rule(0)">用户协议</view>》及《<view class="rule" @tap="go_rule(1)">隐私条款</view>》
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changecode() {
                this.codeurl = config.imgUri + 'inc/checkcode.inc.php?rand=' + Math.random() + '&agentid=' + this
                    .agentid();
            },
            changePassword() {
            ...
            check_name(value) {
                ...
                if (reg.test(value)) {
                    if (value.length > 5) {
                        ...
                        http.setWait(false).post('user.php?act=checkname1', {
                            username: value
                        }).then(res => {
            ...
            check_other(value, name) {
                ...
                if (name == 'randcode') {
                    ...
                    if (value.length > 0) {
                        ...
                        if (value.length == 4) {
                            http.setWait(false).post('user.php?act=checkcode1', {
                                randcode: value
                            }).then(res => {
                                var data = res.data;
            ...
            click_reg() {
                ...
                api.register({
                    ...
                    randcode: this.form.captch,
                    invite_code: this.form.randcode
                }).then(res => {
                    this.checkIn = true;
                    if (res.code == 200) {
                        this.login_success(res);
